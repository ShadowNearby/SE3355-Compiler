...
void RegAllocator::ClearCoalesceMove() {
  ...
  for (const auto instr : assem_instr_->GetInstrList()->GetList()) {
    ...
    auto src = *instr->Use()->GetList().begin();
    auto dst = *instr->Def()->GetList().begin();
    auto src_node = temp_node_map_->Look(src);
    auto dst_node = temp_node_map_->Look(dst);
    if (!coalesced_moves_->Contain(src_node, dst_node)) {
      new_instr_list->Append(instr);
    } else {
      coalesced_moves_->Delete(src_node, dst_node);
    }
  }
