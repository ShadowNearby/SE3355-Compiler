...
gc::TigerHeap *tiger_heap = new gc::DerivedHeap;
...
EXTERNC long *init_array(int size, long init) {
  ...
  tiger_heap->stack += 2;
  if (tiger_heap->main_stack == nullptr) {
    GET_TIGER_STACK(tiger_heap->main_stack);
  }
  int i;
  ...
  if (!a) {
    tiger_heap->GC();
    a = (long *)tiger_heap->AllocArray(allocate_size);
  }
  for (i = 0; i < size; i++)
    a[i] = init;
  return a;
...
EXTERNC int *alloc_record(int size, struct string *descriptor) {
  ...
  int i;
  int *p, *a;
  p = a = (int *)tiger_heap->AllocRecord(size, descriptor->chars,
                                         descriptor->length);
  if (!p) {
    tiger_heap->GC();
    p = a = (int *)tiger_heap->AllocRecord(size, descriptor->chars,
