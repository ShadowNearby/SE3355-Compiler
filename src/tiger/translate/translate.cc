...
std::string PrintTy(type::Ty *ty) {
  ...
  if (typeid(*ty) == typeid(type::NameTy)) {
    ...
    std::string res = "(" + name_ty->sym_->Name() + ")";
    return res;
  }
  return "ErrorInTy";
}
...
tr::ExpAndTy *RecordExp::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  // for gc
  args->Append(new tree::NameExp(temp::LabelFactory::NamedLabel(
      "L" + std::to_string(std::hash<std::string>{}(typ_->Name())))));
...
tr::ExpAndTy *SeqExp::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  if (seq_->GetList().size() == 1) {
    auto exp =
        seq_->GetList().front()->Translate(venv, tenv, level, label, errormsg);
    return new tr::ExpAndTy{new tr::ExExp(exp->exp_->UnEx()), exp->ty_};
  }
...
tr::Exp *FunctionDec::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  for (const auto fun_dec : functions_->GetList()) {
    ...
    auto new_level =
        new tr::Level{new frame::X64Frame{fun_dec->name_, escapes, tys}, level};
...
tr::Exp *VarDec::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
                           ...
                           err::ErrorMsg *errormsg) const {
  auto init_exp_ty = init_->Translate(venv, tenv, level, label, errormsg);
  auto check_pointer = frame::CheckPointer(init_exp_ty->ty_);
  if (typ_) {
    auto type_entry = tenv->Look(typ_);
    check_pointer = check_pointer || frame::CheckPointer(type_entry);
  }
  auto access = check_pointer ? tr::Access::AllocLocal(level, true)
                              : tr::Access::AllocLocal(level, escape_);
  if (check_pointer) {
    level->frame_->AddPointer(access->access_);
  }
...
tr::Exp *TypeDec::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  for (const auto type_name_ty : types_->GetList()) {
    ...
    auto record_ty = dynamic_cast<type::RecordTy *>(actual_ty);
    auto descriptor_label = temp::LabelFactory::NamedLabel(
        "L" + std::to_string(std::hash<std::string>{}(name_ty->sym_->Name())));
    auto descriptor_val = std::string{};
    for (const auto field : record_ty->fields_->GetList()) {
      if (frame::CheckPointer(field->ty_)) {
        descriptor_val += "1";
      } else {
        descriptor_val += "0";
      }
    }
    frags->PushBack(new frame::StringFrag{descriptor_label, descriptor_val});
  }
  return new tr::ExExp(new tree::ConstExp(0));
}
