...
public:
  ...
  void NewFrame(const std::list<bool> &formals, type::TyList *tys) {
    auto formal_it = formals.begin();
    auto ty_it = tys->GetList().begin();
    auto escape = *formal_it;
    auto access = AllocLocal(escape);
    this->formals_.emplace_back(access);
    ++formal_it;
    for (int i = 0; i < tys->GetList().size(); ++i, ++formal_it, ++ty_it) {
      escape = *formal_it;
      auto ty = *ty_it;
      if (CheckPointer(ty)) {
        pointers_.emplace_back(access);
        access = AllocLocal(true);
      } else {
        access = AllocLocal(escape);
      }
      this->formals_.emplace_back(access);
    }
  }
  void AddPointer(frame::Access *access) override {
    pointers_.emplace_back(access);
  }
};
